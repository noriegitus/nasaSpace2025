...
Basado EXACTAMENTE en train_specialists.py y train_judge.py
"""

# Features base (para validación de entrada)
BASE_FEATURES = {
    'fotometria': [
        'koi_duration',
        'koi_depth',
...
# Features que requieren incertidumbre (generarán _sigma, _snr, _rel_unc)
...
# Features completos para cada modelo (incluyendo derivados)
FOTOMETRIA_FEATURES = [
...
# Features para el juez (scores de los 4 especialistas)
JUDGE_FEATURES = [
    'score_fotometria',
    'score_orbital',
    'score_estelar',
    'score_falsos_positivos'
]


def get_base_features(model_type: str) -> list:
    """
    Retorna las características base requeridas para un modelo específico.
    
    Args:
        model_type: Tipo de modelo ('fotometria', 'orbital', 'estelar', 'falsos_positivos', 'judge')
        
    Returns:
        Lista de nombres de características base
    """
    return BASE_FEATURES.get(model_type, [])

def get_uncertainty_features(model_type: str) -> list:
    """
    Retorna las características que requieren cálculo de incertidumbre.
    
    Args:
        model_type: Tipo de modelo ('fotometria', 'orbital', 'estelar', 'falsos_positivos', 'judge')
        
    Returns:
        Lista de nombres de características que requieren incertidumbre
    """
    return UNCERTAINTY_FEATURES.get(model_type, [])

def get_feature_group(model_type: str) -> list:
    """
    Retorna el grupo completo de características para un tipo de modelo específico.
    
    Args:
        model_type: Tipo de modelo ('fotometria', 'orbital', 'estelar', 'falsos_positivos', 'judge')
        
    Returns:
        Lista completa de nombres de características
    """
    feature_map = {
